...
class theme_baz_mod_lesson_renderer extends mod_lesson_renderer {
    ...
    public function lesson_inaccessible($message) {
        global $CFG;
        $output  =  $this->output->box_start('generalbox boxaligncenter wrapper-md');
        $output .=  $this->output->box_start('center wrapper-md');
        $output .=  $message;
    ...
    public function continue_links(lesson $lesson, $lastpageseenid) {
        global $CFG;
        $output = $this->output->box(get_string('youhaveseen','lesson'), 'generalbox boxaligncenter wrapper-md');
        $output .= $this->output->box_start('center wrapper-md');
    ...
    public function display_eol_page(lesson $lesson, $data) {
        ...
        if ($lesson->custom && !$canmanage && (($data->gradeinfo->nquestions < $lesson->minquestions))) {
            $output .= $this->box_start('generalbox boxaligncenter wrapper-md');
        }
        ...
        if ($data->notenoughtimespent !== false) {
            $output .= $this->paragraph(get_string("notenoughtimespent", "lesson", $data->notenoughtimespent), 'center wrapper-md');
        }

        if ($data->numberofpagesviewed !== false) {
            $output .= $this->paragraph(get_string("numberofpagesviewed", "lesson", $data->numberofpagesviewed), 'center wrapper-md');
        }
        if ($data->youshouldview !== false) {
            $output .= $this->paragraph(get_string("youshouldview", "lesson", $data->youshouldview), 'center wrapper-md');
        }
        if ($data->numberofcorrectanswers !== false) {
            $output .= $this->paragraph(get_string("numberofcorrectanswers", "lesson", $data->numberofcorrectanswers), 'center wrapper-md');
        }

        if ($data->displayscorewithessays !== false) {
            $output .= $this->box(get_string("displayscorewithessays", "lesson", $data->displayscorewithessays), 'center wrapper-md');
        } else if ($data->displayscorewithoutessays !== false) {
            $output .= $this->box(get_string("displayscorewithoutessays", "lesson", $data->displayscorewithoutessays), 'center wrapper-md');
        }

        if ($data->yourcurrentgradeisoutof !== false) {
            $output .= $this->paragraph(get_string("yourcurrentgradeisoutof", "lesson", $data->yourcurrentgradeisoutof), 'center wrapper-md');
        }
        ...
        if ($data->reviewlesson !== false) {
            $output .= '<div class="wrapper-md">'.html_writer::link($data->reviewlesson, get_string('reviewlesson', 'lesson'),
        ...
        $output .= '<div class="wrapper-md">'.html_writer::link($url, get_string('returnto', 'lesson', format_string($course->fullname, true)),
                array('class' => 'btn btn-primary')).'</div>';
        ...
        if (has_capability('gradereport/user:view', context_course::instance($course->id))
            ...
            $output .= '<div class="wrapper-md">'.html_writer::link($url, get_string('viewgrades', 'lesson'),
                array('class' => 'btn btn-primary')).'</div>';
        }
        return $output;
    }
