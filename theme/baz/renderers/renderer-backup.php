...
class theme_baz_core_backup_renderer extends core_backup_renderer {
    ...
    public function progress_bar(array $items) {
        ...
        return html_writer::tag('div', join($items), array('class' => 'rui-backup-progress wrapper-md'));
    }
    ...
    public function log_display($loghtml) {
        $out = html_writer::start_div('backup_log wrapper-md');
        $out .= $this->output->heading(get_string('backuplog', 'backup'));
        $out .= html_writer::start_div('backup_log_contents wrapper-md mt-4');
        $out .= $loghtml;
