...
$PAGE->set_url('/local/feepayment/review.php');
...
$username = $DB->get_field('user', 'username', array('id' => $userid));
    ...
    $PAGE->set_title('Review feepayment');
    $PAGE->set_heading('Review feepayment');

    $mform = new review_form();
    $toform = []; 

    // $id='2';
    // $toform = $DB->get_record('local_feepayment', array('id'=> 2 ),'*', MUST_EXIST);
    // $toform = $DB->get_record_sql('local_feepayment', array(userid= 2 AND 'creted' = $d),'*', MUST_EXIST);
    ...
    // echo "id :" .$id."<br>";
    // die;
    $rs = $DB->get_records_sql("SELECT * FROM {local_feepayment} WHERE (userid = '$id' AND created = '$d' )", array(1)) ;
    ...
    if ($mform->is_cancelled()) {
        ...
        redirect($CFG->wwwroot . '/local/feepayment/manage.php');
        ...
        $DB->update_record('local_feepayment', $recordtoinsert);
...
// echo "-local-feepayment-review.php line 119 user <br>";
// print_object($USER);
// die;
// $userfrom = $USER;

// $message = new \core\message\message();
// $message->component = 'local_feepayment'; // Your plugin's name
// $message->name = 'feepayment'; // Your notification name from message.php
...
// echo "-local-feepayment-review.php line 142 user <br>";
...
// ----- end send messgae ----
    ...
    //  redirect($CFG->wwwroot . '/local/feepayment/manage.php', 'You just updated : '. $email);
